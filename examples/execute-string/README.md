...
The basics of executing Gremlin queries that are **NOT** bound using the `ExecuteStringQuery` function in the Grammes client.

## Description

**execute-string** demonstrates how to execute Gremlin queries that are **NOT** bound. Specifically this example shows how to execute them using the `ExecuteStringQuery` function in the Grammes client which takes in a `string` query. Then returns a `[]byte` and `error`. The `[]byte` can be used to *unmarshal* into any kind of expected result such as a list of vertices.
...
- Adds vertex using a string query with no bindings
