...
The basics of counting vertices on the graph using the `VertexCount` function from a Grammes client.

## Description

**vertex-count** demonstrates how to count the vertices on the graph using a Grammes client. Specifically this examples shows how to by using the `VertexCount` function which returns an `int64` for count and `error`.
