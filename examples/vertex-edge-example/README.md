...
The basics of adding edges between vertices and traversing through edges to find relative vertices.

## Description

**vertex-edge-example** demonstrates how to add edges between vertices using the `Vertex` struct and `AddEdge` function which will take a Grammes client as a parameter along with the `ID` of the outgoing Vertex, `label`, and `properties`.
