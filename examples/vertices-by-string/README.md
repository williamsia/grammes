...
The basics of getting vertices using the `VerticesByString` function from a Grammes client.

## Description

**vertices-by-string** demonstrates how to receive vertices on the graph using a Grammes client. Specifically this examples shows how to by using the `VerticesByString` function which takes in a `string` query meant to receive vertices. Then returns a slice of `Vertex` and `error`.
...
- Gathers the vertex structs using a string query
